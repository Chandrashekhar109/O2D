...
const APP = {
  ...
  STATUS: {
    ...
    AWAITING_MATERIAL: 'Awaiting Material Availability',
  ...
  /*
   * Material roles map 1:1 onto the raw materials an order needs. A user
   * with Role = 'yarn' confirms Yarn availability; Role = 'fabric' confirms
   * Fabric availability. Both must be marked done before production
   * planning for an order can start.
   */
  MATERIALS: [
    { key: 'yarn', label: 'Yarn', role: 'yarn' },
    { key: 'fabric', label: 'Fabric', role: 'fabric' }
  ],
...
/* ---------------------------------------------------------------------------
   RAW MATERIAL AVAILABILITY HELPERS
   ------------------------------------------------------------------------ */

function ensureMaterialSheet_() {
  const sh = ensureColumns_(
    ensureSheet_(APP.SHEETS.MATERIAL, APP.MATERIAL_COLUMNS),
    APP.MATERIAL_COLUMNS
  );
  sh.setFrozenRows(1);
  return sh;
}

/* One row per order x material (Yarn / Fabric). */
function readMaterialRows_() {
  const sh = ensureMaterialSheet_();
...
/* { 'Yarn': row|null, 'Fabric': row|null } for one order. */
function materialByKindForUUID_(uuid) {
  const clean = normalize_(uuid);
  const out = {};

  APP.MATERIALS.forEach(function(m) { out[m.label] = null; });

  if (!clean) return out;

  const rows = readMaterialRows_();

  rows.forEach(function(r) {
    if (normalize_(r['UUID']) !== clean) return;
    APP.MATERIALS.forEach(function(m) {
      if (normalize_(r['Material']) === m.label) out[m.label] = r;
    });
  });
...
/*
 * Production planning for an order unlocks only when both Yarn and Fabric
 * availability have been confirmed (Done = Yes). Returns true once both
 * records exist and are done.
 */
function isMaterialReady_(uuid) {
  const byKind = materialByKindForUUID_(uuid);
  return APP.MATERIALS.every(function(m) {
    const rec = byKind[m.label];
    return !!rec && isYesValue_(rec['Done']);
  });
}

/* Which material a session is allowed to confirm (Yarn/Fabric) or ''. */
function materialForSession_(session) {
  if (!session) return '';
  const role = normalize_(session.role).toLowerCase();
  const found = APP.MATERIALS.filter(function(m) {
    return m.role === role;
  })[0];
...
/*
 * Frontend-friendly summary of an order's raw material availability:
 * { 'Yarn': {..}|null, 'Fabric': {..}|null, 'Ready': bool }
 */
function materialSummaryForUUID_(uuid) {
  const byKind = materialByKindForUUID_(uuid);
  const summary = {
    'Yarn': byKind['Yarn'],
    'Fabric': byKind['Fabric'],
    'Ready': isMaterialReady_(uuid)
  };
  return summary;
}
...
function apiSendToGodown(token, uuid, externalInvQty, godown) {
  return withError_(function() {
    ...
    /*
     * Appending to the godown sheet must be driven by Column A, not by
     * getLastRow(). The godown sheet contains checkboxes in some columns and
     * checkboxes count as "data", so getLastRow() can point far below the
     * last actual order row and new rows would be written into a gap.
     */
    const lastColumnARow = (function() {
      const filledRows = godownSheet.getLastRow();
      if (filledRows < 1) return 1;
      const colA = godownSheet.getRange(1, 1, filledRows, 1).getValues();
...
function apiDecideProduction(
  ...
  return withError_(function() {
     ...
     * Send to production.
     *
     * Planning details (grid, revo, desc, flow, dates, place, allocation)
     * are intentionally NOT collected here anymore. The order first waits
     * for the Yarn and Fabric users to confirm raw material availability;
     * only then does it unlock the Production Planning view where the
     * sales person builds the full plan.
     */
    ...
    setIfColumnExists_(
      ...
      rowNumber,
      'Production Decision',
      'Sent to production — awaiting raw material availability'
    );
...
function apiGetMyProductionPlans(token) {
  return withError_(function() {
    ...
    /*
     * Every plan row carries the raw material availability summary so the
     * Production Planning view can show Yarn/Fabric status next to the plan.
     */
    plans.forEach(function(plan) {
      const u = normalize_(plan['UUID']);
      if (!u) return;
      plan['Material'] = materialSummaryForUUID_(u);
    });

    /*
     * Orders that were sent to production but do NOT have a plan row yet.
     * They wait for the Yarn/Fabric availability to be confirmed and then
     * for the sales person to plan them. They belong in the Production
     * Planning list (activePlans) but NOT in the Process Board, which only
     * shows real plans.
     */
    ...
    ordersData.rows.forEach(function(order) {
      ...
      const uuid = normalize_(order['UUID']);
      if (!uuid || !myUUIDs[uuid]) return;
      if (hasPlan[uuid]) return;

      const status = normalize_(order['Status']);
      if (
        status !== APP.STATUS.AWAITING_MATERIAL &&
        status !== APP.STATUS.SENT_TO_PRODUCTION
      ) {
        return;
      }

      awaitingPlanning.push({
        'UUID': uuid,
        ...
        'Desc': '',
        'Greige/Finish': '',
        'Remaining Qty': order['Remaining Qty'] || '',
        'Production Place': '',
        'Production Units': '',
        'Outside Parties': '',
        'Flow Steps': '',
        ...
        'Current Step': '',
        'Lifecycle Status': '',
        'Material': materialSummaryForUUID_(uuid),
        '__awaitingPlanning': true,
        '__row': order.__row
      });
    });
    ...
    return ok_({
      plans: plans,
      activePlans: activePlans.concat(awaitingPlanning),
      awaitingPlanning: awaitingPlanning
    });
  });
}
...
function apiUpdatePlanDates(token, uuid, dates) {
  return withError_(function() {
    ...
    const mendingType = dates.mendingType !== undefined
      ...
      : (plan ? normalize_(plan['Mending Type']) : '');

    const currentSteps = dates.flowSteps !== undefined
      ? dates.flowSteps
      : (plan ? String(plan['Flow Steps'] || '').split(',') : []);
      ...
      merged[step.column] = flowSteps.indexOf(step.key) === -1
        ...
        : normalize_(
            dates[step.column] !== undefined
              ? dates[step.column]
              : (plan ? plan[step.column] : '')
          );
    });

    /*
     * A plan is created here for the first time only when the raw material
     * availability (Yarn + Fabric) has been confirmed by the material users.
     * Grid / Revo / Desc / Greige-Finish are captured during this planning.
     */
    if (isNew) {
      if (!isMaterialReady_(uuid)) {
        throw new Error(
          'Raw material availability pending. Both Yarn and Fabric ' +
          'availability must be confirmed before production planning.'
        );
      }

      const grid = normalize_(dates.grid);
      const revo = normalize_(dates.revo);
      const desc = normalize_(dates.desc);
      const greigeFinish = normalize_(dates.greigeFinish);

      if (!grid || !revo || !desc || !greigeFinish) {
        throw new Error(
          'Grid, Revo, Desc and Greige/Finish are required to create the plan.'
        );
      }

      const remainingQty = normalize_(found.row['Remaining Qty']) ||
        number_(found.row['Order QTY']);
      ...
      const ordersSheet = getSheet_(APP.SHEETS.ORDERS);
      const header = found.header;
      const rowNumber = found.row.__row;

      if (normalize_(found.row['Status']) === APP.STATUS.AWAITING_MATERIAL) {
        setIfColumnExists_(
          ordersSheet, header, rowNumber, 'Status',
          APP.STATUS.SENT_TO_PRODUCTION
        );
        setIfColumnExists_(
          ordersSheet, header, rowNumber, 'Production Decision',
      ...
      upsertProductionPlan_({
        ...
        remainingQty: remainingQty,
        stockStage: found.row['Stock Current Stage'] || '',
        dates: merged,
        flowSteps: flowSteps,
        mendingType: mendingType,
        place: split.place,
    ...
    const grid = dates.grid !== undefined
      ? normalize_(dates.grid)
      : normalize_(plan['Grid']);
    const revo = dates.revo !== undefined
      ? normalize_(dates.revo)
      : normalize_(plan['Revo']);
    const desc = dates.desc !== undefined
      ? normalize_(dates.desc)
      : normalize_(plan['Desc']);
    const greigeFinish = dates.greigeFinish !== undefined
      ? normalize_(dates.greigeFinish)
      : normalize_(plan['Greige/Finish']);
...
/* ============================================================================
   23. RAW MATERIAL AVAILABILITY (YARN / FABRIC)
   ========================================================================== */

/*
 * Orders that have been sent to production and now wait for the Yarn and
 * Fabric users to confirm raw material availability. Used by the material
 * users' view and shown to the sales person in Production Planning.
 *
 * Scope: any role may read it, but the material users (yarn / fabric) see
 * every such order — material availability is a shared resource, it is not
 * scoped to the sales person who owns the order.
 */
function apiGetMaterialAvailability(token) {
  return withError_(function() {
    const session = requireSession_(token);
    ...
    ordersData.rows.forEach(function(order) {
      ...
      const byKind = materialByKindForUUID_(uuid);
      const hasMaterialRow = APP.MATERIALS.some(function(m) {
        return byKind[m.label] && isYesValue_(byKind[m.label]['Done']);
      });

      /*
       * Open orders that are waiting for material confirmation or already
       * in production planning form the live "awaiting / confirmed" list.
       */
      if (!closed && (
        status === APP.STATUS.AWAITING_MATERIAL ||
        status === APP.STATUS.SENT_TO_PRODUCTION
        ...
        obj['Yarn Record'] = null;
        obj['Fabric Record'] = null;

        APP.MATERIALS.forEach(function(m) {
          obj[m.label + ' Record'] = byKind[m.label];
        });

        obj['Material Ready'] = isMaterialReady_(uuid);
        obj.__row = order.__row;
        orders.push(obj);
        return;
      }
      ...
      /*
       * Completed (closed) orders that the material user confirmed on are
       * surfaced in a separate "Completed" list so the material view keeps
       * a history without cluttering the live pending/confirmed panels.
       */
      if (closed && hasMaterialRow) {
        const obj = {};
        APP.ORDER_BASE_COLUMNS.concat(APP.ORDER_WORKFLOW_COLUMNS)
          .forEach(function(col) {
            if (order[col] !== undefined) obj[col] = order[col];
        ...
        obj['Yarn Record'] = byKind['Yarn'] || null;
        obj['Fabric Record'] = byKind['Fabric'] || null;
        obj['Material Ready'] = isMaterialReady_(uuid);
        obj['Closed Timestamp'] = order['Closed Timestamp'] || '';
        obj['Closed By'] = order['Closed By'] || '';
        obj.__row = order.__row;
        completed.push(obj);
      }
    });

    return ok_({ orders: orders, completed: completed });
  });
}

function apiGetMaterialAvailabilityJson(token) {
  return jsonString_(apiGetMaterialAvailability(token));
}

/*
 * YARN / FABRIC user — records the availability for their material for one
 * order. Submitting always marks the record Done. Re-submitting overwrites
 * the previous entry, so a wrong figure can be corrected.
 *
 *   payload: { qtyAvailable, provideDate, status, remarks }
 *
 *   status      = Available | Partial | Not Available
 *   provideDate = date by which the full requirement will be in hand
 *                 (required for Partial / Not Available)
 */
function apiSubmitMaterialAvailability(token, uuid, payload) {
  return withError_(function() {
    const session = requireSession_(token);

    const material = materialForSession_(session);
    if (!material) {
      throw new Error(
        'Only the Yarn and Fabric users can record material availability.'
      );
    }
    ...
    const allowedStatuses = [
      APP.MATERIAL_STATUS.AVAILABLE,
      APP.MATERIAL_STATUS.PARTIAL,
      APP.MATERIAL_STATUS.NOT_AVAILABLE
    ];
    if (allowedStatuses.indexOf(status) === -1) {
      throw new Error('Status must be Available, Partial or Not Available.');
    }

    const qtyAvailable = number_(payload.qtyAvailable);
    if (qtyAvailable < 0) {
      throw new Error('Available qty cannot be negative.');
    }

    if (
      (status === APP.MATERIAL_STATUS.PARTIAL ||
        status === APP.MATERIAL_STATUS.NOT_AVAILABLE) &&
      !normalize_(payload.provideDate)
    ) {
      throw new Error(
        'A provide date is required when material is partial or unavailable.'
      );
    }

    const provideDate = normalize_(payload.provideDate);

    const requiredQty = number_(order['Order QTY']);

    const sh = ensureMaterialSheet_();
    const header = getHeader_(sh);
    const data = readMaterialRows_();

    let targetRow = -1;

    data.forEach(function(r) {
      if (
        normalize_(r['UUID']) === uuid &&
        normalize_(r['Material']) === material
      ) {
        targetRow = r.__row;
      }
    });

    const values = APP.MATERIAL_COLUMNS.map(function(col) {
      switch (col) {
        case 'UUID': return uuid;
        case 'Order No': return order['Order No'] || '';
        case 'Material': return material;
        case 'Qty Required': return requiredQty;
        case 'Qty Available': return qtyAvailable;
        case 'Provide Date': return provideDate;
        case 'Status': return status;
        case 'Done': return 'Yes';
        case 'Provided By': return session.name || session.username || '';
        case 'Remarks': return normalize_(payload.remarks);
        case 'Timestamp': return timestamp_();
        default: return '';
      }
    });
...
function apiSubmitMaterialAvailabilityJson(token, uuid, payload) {
  return jsonString_(apiSubmitMaterialAvailability(token, uuid, payload));
}


/* ============================================================================
   24. DEPARTMENT DASHBOARD
   ========================================================================== */
...
function apiCreateUser(token, user) {
  return withError_(function() {
    ...
    if (['admin', 'sales', 'department', 'yarn', 'fabric'].indexOf(role) === -1) {
      throw new Error('Invalid role. Choose admin, sales, department, yarn or fabric.');
    }
